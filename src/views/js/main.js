...
var resizePizzas = function(size) {
  ...
  function changePizzaSizes(size) {
    ...
    // Retrieve all Pizzas and change it's width
...
function logAverageFrame(times) {   // times is the array of User Timing measurements from updatePositions()
  ...
  for (var i = numberOfEntries - 1; i > numberOfEntries - 11; i--) {
    console.log('duration' + times[i].duration);
...
function updatePositions() {
 ...
 //Cache all moving pizzas using class Name 
  var items = document.getElementsByClassName('mover'); 

  //Cache Length
  var length = items.length;                            
  
  //Calculation is moved out of the loop
  var scrollTop = document.body.scrollTop / 1250;  

 for (var i = 0; i < length; i++) {
    //Calculate positions
    var phase = Math.sin(scrollTop + (i % 5)) * 100;  
    
    //Change Left property of each pizzas
